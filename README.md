...
For a version that doesn't require Python, VS Code, or a terminal to run,
build a standalone `JARVIS.exe`:

```bat
scripts\build_windows.bat
```

This installs PyInstaller (from `requirements-build.txt`) into your existing
`.venv` and produces `dist\JARVIS\JARVIS.exe`. Copy `.env.example` next to the
`.exe` as `.env` and add your `GEMINI_API_KEY` (or launch it once and it will
